"""
Phase 3 unification rules for dynamics and lyrics in satb-split.
"""
...
def apply_phase3_unification(voices_dict):
    """Apply Phase 3 unification rules for dynamics and lyrics across SATB parts."""
    print("Analyzing dynamics and lyrics for Phase 3 unification...")
    ...
    unify_dynamics_phase3(voices_dict, soprano_dynamics, alto_dynamics, tenor_dynamics, bass_dynamics)
    ...
    unify_lyrics_phase3(voices_dict, soprano_lyrics, alto_lyrics, tenor_lyrics, bass_lyrics)
...
def unify_dynamics_phase3(voices_dict, soprano_dynamics, alto_dynamics, tenor_dynamics, bass_dynamics):
...
def unify_lyrics_phase3(voices_dict, soprano_lyrics, alto_lyrics, tenor_lyrics, bass_lyrics):
...
def dynamics_match(dynamics1, dynamics2):
    ...
    for d1, d2 in zip(dynamics1, dynamics2):
        if (d1['measure'] != d2['measure'] or 
            abs(d1['offset'] - d2['offset']) > 0.1 or 
            d1['value'] != d2['value']):
            return False
    return True
        ...
        for dynamic_info in source_dynamics:
            ...
            for measure in target_part.getElementsByClass(music21.stream.Measure):
                if measure.number == measure_num:
                    # Create a proper copy of the dynamic object
